...
def rotation_3d(omega, theta):
    ...
    w= skew_3d(omega)
    q=theta
    l = math.sqrt(np.multiply(omega[0],omega[0])+np.multiply(omega[1],omega[1])+np.multiply(omega[2],omega[2]))
    i= np.array([[1,0,0],[0,1,0],[0,0,1]])
    R = i + np.multiply((w /l),(math.sin(l*q))) + np.multiply((np.matmul(w,w)/l**2),(1-math.cos(l*q)))
    return R
...
def hat_3d(xi):
    ...
    # YOUR CODE HERE
    hat_3d = np.array([[0, -omega[5], omega[4], omega[0]], [omega[5], 0, -omega[3], omega[1]], [-omega[4], omega[3], 0, omega[2]], [0, 0, 0, 0]])
...
def homog_3d(xi, theta):
    ...
    v = vt.reshape(3, 1)
    omega = xi[3:]
    wt = np.array([[omega[0]], [omega[1]], [omega[2]]])
    w = skew_3d(omega)
    l = np.linalg.norm(omega)
    i = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
    R = rotation_3d(omega, theta)
    U = (1 / pow(l, 2)) * (((np.subtract(i, R)) @ (w @ v)) + (omega.reshape(3, 1) @ omega.reshape(1, 3) @ v * theta))
